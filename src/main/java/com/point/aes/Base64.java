...
public class Base64 {
    ...
    /**
     * Used to detcnpeine how many bits data contains.
     */
    ...
    public static byte[] decodeNoSpase(byte[] pArray) {
        return decodeBase64NoSapce(pArray);
    }
    ...
    public static byte[] decodeBase64NoSapce(byte[] base64Data) {
        // RFC 2045 requires that we discard ALL non-Base64 characters
        base64Data = discardNonBase64NoSpace(base64Data);
    ...
    static byte[] discardNonBase64NoSpace (byte[] data) {
        byte groomedData[] = new byte[data.length];
        int bytesCopied = 0;

        for (int i = 0; i < data.length; i++) {
            boolean base64 = isBase64(data[i]);
            if(!base64)throw new RuntimeException("包含非BASE64字符");
            groomedData[bytesCopied++] = data[i];
        }
